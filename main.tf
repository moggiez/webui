...
module "cloudfront_cert" {
  ...
  domain_name = var.env != "prod" ? "${var.env}.${var.custom_domain}" : "www.${var.custom_domain}"
  zone_id     = data.aws_route53_zone.custom_domain_zone.zone_id

  tags = {
    Name = "CloudFront ${var.env} - ${var.custom_domain}"
  }
...
module "tf_next" {
  ...
  deployment_name = "terraform-next-js-${var.env}-moggies-io"
  providers = {
...
resource "aws_cloudfront_distribution" "distribution" {
  ...
  comment             = "terraform-next-js-${var.env}-moggies-io"
  aliases             = ["${var.env}.${var.custom_domain}"]
...
output "custom_domain_name" {
  value = aws_route53_record.cloudfront_alias_domain.name
}
